...
<div class="col-12">
  <div class="alert alert-info small mb-0">
    <strong>Editing tip:</strong> choose <strong>Shared</strong> to update all levels in this unit family (e.g. Diploma/Certificate/Artisan together). Choose <strong>This level only</strong> to override just this specific programme.
  </div>
</div>
<div class="col-12"><label class="form-label">Unit Overview</label><textarea name="programme_overview" rows="4" class="form-control rich-editor"><?= e($programmeContent['overview'] ?? '') ?></textarea></div>
<div class="col-md-6"><label class="form-label">Unit Objectives (one per line)</label><textarea name="programme_objectives" rows="6" class="form-control"><?= e($programmeContent['objectives'] ?? '') ?></textarea></div>
<div class="col-md-6"><label class="form-label">Unit Content Areas (one per line)</label><textarea name="programme_content_areas" rows="6" class="form-control"><?= e($programmeContent['content_areas'] ?? '') ?></textarea></div>
...
<div class="col-md-6"><label class="form-label">Why Study This Unit? (one per line)</label><textarea name="programme_why_study" rows="6" class="form-control"><?= e($programmeContent['why_study'] ?? '') ?></textarea></div>
...
<div class="col-md-6"><label class="form-label">Current Homepage Card Image URL</label><div class="input-group"><input id="programme_home_card_image_current" class="form-control" value="<?= e((string)($programmeHomeCardImage ?? 'Not set')) ?>" readonly><button type="button" class="btn btn-outline-danger" onclick="document.getElementById('programme_home_card_image_current').value='Not set';document.querySelector('[name=programme_home_card_image_delete]').value='1'"><i class="bi bi-x"></i></button></div><input type="hidden" name="programme_home_card_image_delete" value="0"></div>
...
<div class="col-md-4"><label class="form-label">Role</label><select name="role" class="form-select"><?php foreach(['super_admin' => 'Super Admin', 'junior_admin' => 'Senior Admin', 'editor' => 'Editor', 'viewer' => 'Viewer', 'registrar' => 'Registrar', 'teacher' => 'Teacher'] as $roleValue => $roleLabel): ?><option value="<?= e($roleValue) ?>" <?= (($row['role'] ?? 'teacher') === $roleValue) ? 'selected' : '' ?>><?= e($roleLabel) ?></option><?php endforeach; ?></select></div>
...
<div class="col-md-4"><label class="form-label">Session</label><select name="session_id" class="form-select" required><option value="">Select session</option><?php foreach(($sessions ?? []) as $s): ?><option value="<?= e((string)$s['id']) ?>" <?= ((string)($row['session_id'] ?? '') === (string)$s['id']) ? 'selected' : '' ?>><?= e((string)$s['name']) ?></option><?php endforeach; ?></select></div>
...
<div class="col-md-4"><label class="form-label">Unit</label><select name="course_id" class="form-select" required><option value="">Select unit</option><?php foreach(($courses ?? []) as $course): ?><option value="<?= e((string)$course['id']) ?>" <?= ((string)($row['course_id'] ?? '') === (string)$course['id']) ? 'selected' : '' ?>><?= e(trim((string)($course['code'] ?? '') . ' ' . (string)$course['title'])) ?></option><?php endforeach; ?></select></div>
<div class="col-md-4"><label class="form-label">Marks (%)</label><input name="marks" type="number" min="0" max="100" step="0.01" class="form-control" value="<?= e((string)($row['marks'] ?? '')) ?>" placeholder="e.g 72.5" required></div>
<div class="col-md-4"><label class="form-label">Grading System (optional)</label><select name="grading_system_id" class="form-select"><option value="">Auto by marks range</option><?php foreach(($gradingSystems ?? []) as $system): ?><option value="<?= e((string)$system['id']) ?>" <?= ((string)($row['grading_system_id'] ?? '') === (string)$system['id']) ? 'selected' : '' ?>><?= e((string)$system['name']) ?> (<?= e((string)($system['exam_type_name'] ?? 'Default')) ?>)</option><?php endforeach; ?></select></div>
<div class="col-md-4"><label class="form-label">Grade</label><input name="grade" class="form-control" value="<?= e($row['grade'] ?? '') ?>" placeholder="Auto-filled from marks" readonly></div>
...
<div class="col-md-4"><label class="form-label">Visible</label><div class="form-check mt-2"><input class="form-check-input" type="checkbox" name="is_visible" value="1" <?= (!isset($row['id']) || !empty($row['is_visible'])) ? 'checked' : '' ?>><label class="form-check-label">Show on website</label></div></div>
...
<div class="col-md-3"><label class="form-label">Visible</label><div class="form-check mt-2"><input class="form-check-input" type="checkbox" name="is_visible" value="1" <?= (!isset($row['id']) || !empty($row['is_visible'])) ? 'checked' : '' ?>><label class="form-check-label">Show on website</label></div></div>
<?php else: ?>
